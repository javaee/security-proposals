 ...
 * validate an instance of {@link javax.security.identitystore.credential.Credential}.
 *
...
public class CredentialValidationResult {
    ...
    public enum Status {
         ...
         * no suitable {@link javax.security.identitystore.credential.CredentialValidator} could be found.
         */
        NOT_VALIDATED,
     ...
     * @param identityStore Identity store which performed the validation.
\     */
    public CredentialValidationResult(
        ...
        this(status, callerName, getCallerGroups(status,callerName,identityStore),
     ...
     * <code>null</code> if {@link #getStatus} does not return {@link Status#VALID VALID}.
     */
     ...
     *
     * @return The list of groups that the specified Caller is in, empty if none.
     * <code>null</code> if {@link #getStatus} does not return {@link Status#VALID VALID}
     * or if the identity store does not support groups.
     */
    public List<String> getCallerGroups() { return groups; }

    /**
     * Determines the list of roles that the specified caller is in,
     ...
     * <code>null</code> if {@link #getStatus} does not return {@link Status#VALID VALID}
     * or if the identity store does not support roles.
     */
    public List<String> getCallerRoles() { return roles; }


    private static List<String> getCallerGroups(Status status, String callerName, IdentityStore identityStore)  {
    ...
    private static List<String> getCallerRoles(Status status, String callerName, IdentityStore identityStore)  {
        if (null == identityStore)
            throw new NullPointerException("identityStore");

        if (VALID == status) {
            return identityStore.getCallerRoles(callerName);
        } else {
            return null;
        }
    }
